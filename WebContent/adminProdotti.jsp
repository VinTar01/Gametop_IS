...
<head>
 <!-- Box icons -->
  ...
  <link rel="stylesheet" href="styleNavBar.css" />
    ...
    <title>Admin | Gametop</title>
		
		<script>

	function validazioneProdotto() 
	{
		var codiceProdotto=document.forms['InserimentoProdotto'].codiceProdotto.value;
		 if(!/^[a-zA-Z0-9]{5,5}/.test(codiceProdotto)){
...
if(r==null){
	response.sendRedirect("http://localhost:8080/GametopFV/loginPersonale.jsp");
}
...
<h2>Elenco Prodotti</h2>
	<tr>
		<th>Codice <a href="<%=response.encodeURL("AdminProdotti?sort=codiceProdotto") %>"></a></th>
	...
	%>
			<tr>
				...
				<td>
					<a href="<%=response.encodeURL("AdminProdotti?action=details&id="+bean.getCodiceProdotto()) %>">Dettagli</a>
					<a href="<%=response.encodeURL("AdminProdotti?action=delete&id="+bean.getCodiceProdotto()) %>">Elimina</a>
				</td>
			</tr>
	<% 
			}
...
%>
	...
	<table class="container">
		...
		<tr>
			...
			<td><%=product.getQuantitaProdotto() %></td>
	...
	<form class="container-form" name="ModificaProdotto" onsubmit="return validazioneModificaProdotto()" action="<%=response.encodeURL("AdminProdotti")%>" method="POST">
			...
			<label for="quantitaProdotto">Quantità:</label><br>
			<input id="quantitaProdotto" name="quantitaProdotto" type="number" min="1" required value="<%=product.getQuantitaProdotto()%>"><br>
...
<section class="container">
	<form class="container-form" name="InserimentoProdotto" onsubmit="return validazioneProdotto()" action="<%=response.encodeURL("AdminProdotti")%>" method="POST">
			...
			<label for="codiceProdotto">Codice:</label><br>
			<input id="codiceProdotto" name="codiceProdotto" type="text" maxlength="5" placeholder="enter codice" required><br>
			...
			<label for="quantitaProdotto">Quantita:</label><br>
			<input id="quantitaProdotto" name="quantitaProdotto" type="number" min="0" required><br>
