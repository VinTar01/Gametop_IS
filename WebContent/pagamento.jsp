...
<script type="text/javascript">
		if (window.performance && window.performance.navigation.type === window.performance.navigation.TYPE_BACK_FORWARD) {
			window.location.replace("http://localhost:8080/GametopFV/successoOrdine.jsp");
		}
    </script>
...
<body>
	...
	<h2 class="carta">Ciao <%=nome%> compila i campi per terminare l'acquisto</h2>
	...
	<script type="text/javascript">$(function(){
	  ...
	  }).on("keydown input", function(){ //la funzione viene chiamata ogni volta che l'utente preme un tasto o inserisce un input
	    
	    $(".card_number").text($(this).val()); //aggiorna il testo dell'elemento con classe "card_number" con il valore attuale dell'input
	    
	    if(event.key >= 0 && event.key <= 9){ //verifica se il tasto premuto è un numero (da 0 a 9).
	    //Verifica se la lunghezza attuale dell'input è 4, 9 o 14 caratteri. 
	    //Queste lunghezze corrispondono alle posizioni in cui solitamente vengono aggiunti spazi 
	    //nelle carte di credito per migliorare la leggibilità.
	  ...
	  //Nome
	  $(".inputname").keyup(function(){ //associa una funzione all'evento di rilascio del tasto per l'elemento di input del nome. 
	    $(".fullname").text($(this).val());  //aggiorna il testo dell'elemento con classe "fullname" con il valore attuale dell'input del nome
	    if($(".inputname").val().length === 0){ //Verifica se la lunghezza dell'input del nome è zero e, in tal caso, imposta il testo sulla stringa "FULL NAME
	  ...
	  $(".expire").keypress(function(event){
	      ...
	      if($(this).val().length === 1){
	        ...
	        if(event.key == 1 || event.key == 0){//se iniza con questi numeri va avanti
	          month = event.key;
	          return event.charCode;
	        }else{
